...
// Direct search function imports (bypass HTTP routes — no URL or auth issues)
...
/**
 * SEARCH STEP — Calls search functions directly (no HTTP, no auth needed)
...
async function executeSearchStep(
  ...
  switch (source) {
    case "google":
    case "web": {
      const webResults = await searchWeb(query, { num });
      results = webResults || [];
      break;
    }
